...
void TreeModel::addNodesToList(QList<TreeItem*> * parents) {
	...
	//TODO type list in Options
	//	Manager::GraphManager * manager = Manager::GraphManager::getInstance();
	//	QMap<qlonglong, Graph::Type*> * nodes =
	//			manager->getActiveGraph()->getTypes();
	//
	//	QMap<qlonglong, Graph::Type*>::iterator iterator;
	//
	...
	//		Graph::Type * node = static_cast<Graph::Type*> (iterator.value());
	//		QMap<QString, QString> * settings = node->getSettings();
	//		QMap<QString, QString>::iterator settingsIterator;
	//
	//		QString data;
	//		//postupne sa data zlozia
	//		for (settingsIterator = settings->begin(); settingsIterator
	//				!= settings->end(); settingsIterator++) {
	//			if (settingsIterator != settings->begin())
	//				data.append(";");
	//			data.append(QString("%1,%2").arg(settingsIterator.key()).arg(
	//					settingsIterator.value()));
	//		}
	//
