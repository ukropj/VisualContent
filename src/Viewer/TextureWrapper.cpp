...
{
	if(nodeTexture == NULL)
		nodeTexture = DataHelper::readTextureFromFile(Util::Config::getValue("Viewer.Textures.Node"));
	return nodeTexture;
}
...
osg::ref_ptr<osg::Texture2D> TextureWrapper::getOrientedEdgeTexture()
{
	if(orientedEdgeTexture == NULL)
	{
		...
		{
			stringstream os;
			os << Util::Config::getValue("Viewer.Textures.OrientedEdgePrefix").toStdString();
			os << x;
			os << Util::Config::getValue("Viewer.Textures.OrientedEdgeSuffix").toStdString();
...
{
	if(edgeTexture == NULL)
		edgeTexture = DataHelper::readTextureFromFile(Util::Config::getValue("Viewer.Textures.Edge"));
	return edgeTexture;
}
