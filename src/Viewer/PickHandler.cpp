...
bool PickHandler::handleKeyDown(const osgGA::GUIEventAdapter& ea,
		osgGA::GUIActionAdapter& aa) {
	if (ea.getKey() == osgGA::GUIEventAdapter::KEY_Control_R || ea.getKey()
			== osgGA::GUIEventAdapter::KEY_Control_L) {
		isCtrlPressed = true;
...
bool PickHandler::handleDrag(const osgGA::GUIEventAdapter& ea,
		osgGA::GUIActionAdapter& aa) {
	_mX = ea.getX();
	_mY = ea.getY();
...
bool PickHandler::handlePush(const osgGA::GUIEventAdapter& ea,
		osgGA::GUIActionAdapter& aa) {
	if (ea.getButtonMask() == osgGA::GUIEventAdapter::LEFT_MOUSE_BUTTON) {
		...
		if (pickMode == PickMode::MULTI) {
			...
			return pick(ea.getXnormalized() - 0.00005f, ea.getYnormalized()
					- 0.00005f, ea.getXnormalized() + 0.00005f,
					ea.getYnormalized() + 0.00005f, viewer);
		}
	}
...
void PickHandler::unselectPickedEdges(osg::ref_ptr<Model::Edge> edge) {
	if (edge == NULL) {
		QLinkedList<osg::ref_ptr<Model::Edge> >::const_iterator i =
