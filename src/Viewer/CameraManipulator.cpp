...
Vwr::CameraManipulator::CameraManipulator()
{
    ...
    _distance = 1.0f;
    _trackballSize = Util::Config::getValue("Viewer.CameraManipulator.Sensitivity").toFloat();
    _zoomDelta = 0.4f;

	speedDecelerationFactor = 0.4f;
	maxSpeed = Util::Config::getValue("Viewer.CameraManipulator.MaxSpeed").toFloat();
	speedEpsilon = 0.02f;
