...
Vwr::CameraManipulator::CameraManipulator() {
	...
	_distance = 1.0f;
	_trackballSize = Util::Config::getValue(
			"Viewer.CameraManipulator.Sensitivity").toFloat();
	_zoomDelta = 0.4f;

	speedDecelerationFactor = 0.4f;
	maxSpeed
			= Util::Config::getValue("Viewer.CameraManipulator.MaxSpeed").toFloat();
	speedEpsilon = 0.02f;
...
void Vwr::CameraManipulator::home(const GUIEventAdapter& ea,
		GUIActionAdapter& us) {
	home(ea.getTime());
	us.requestRedraw();
...
bool Vwr::CameraManipulator::handle(const GUIEventAdapter& ea,
		GUIActionAdapter& us) {
...
bool Vwr::CameraManipulator::handleScroll(const osgGA::GUIEventAdapter& ea,
...
bool Vwr::CameraManipulator::handlePush(const osgGA::GUIEventAdapter& ea,
		osgGA::GUIActionAdapter& us) {
...
bool Vwr::CameraManipulator::handleFrame(const osgGA::GUIEventAdapter& ea,
		osgGA::GUIActionAdapter& us) {
...
osg::Matrixd Vwr::CameraManipulator::getInverseMatrix() const {
	return osg::Matrixd::translate(-_center) * osg::Matrixd::rotate(
			_rotation.inverse())
			* osg::Matrixd::translate(0.0, 0.0, -_distance);
}

void Vwr::CameraManipulator::computePosition(const osg::Vec3& eye,
	...
	osg::Matrix rotation_matrix(s[0], u[0], -f[0], 0.0f, s[1], u[1], -f[1],
...
bool Vwr::CameraManipulator::calcMovement() {
	...
	if (_ga_t0->getEventType() == GUIEventAdapter::SCROLL) {
		...
		case osgGA::GUIEventAdapter::SCROLL_2D:
			...
			dx = _ga_t0->getScrollingDeltaX() / ((_ga_t0->getXmax()
					- _ga_t0->getXmin()) * 0.5f);
			dy = _ga_t0->getScrollingDeltaY() / ((_ga_t0->getYmax()
					- _ga_t0->getYmin()) * 0.5f);
	...
	double throwScale =
			(_thrown && _ga_t0.valid() && _ga_t1.valid()) ? _delta_frame_time
					/ (_ga_t0->getTime() - _ga_t1->getTime()) : 1.0;
	...
	} else if (buttonMask == (GUIEventAdapter::LEFT_MOUSE_BUTTON
...
void Vwr::CameraManipulator::setTrackballSize(float size) {
	_trackballSize = size;
	osg::clampBetweenRange(_trackballSize, 0.1f, 1.0f,
			"CameraManipulator::setTrackballSize(float)");
}
 ...
 * 
 */
bool Vwr::CameraManipulator::handleKeyUp(const osgGA::GUIEventAdapter& ea,
		osgGA::GUIActionAdapter & us) {
	switch (ea.getKey()) {
 ...
 * 
 */
bool Vwr::CameraManipulator::handleKeyDown(const osgGA::GUIEventAdapter &ea,
 ...
 * 
 */
void Vwr::CameraManipulator::frame(const osgGA::GUIEventAdapter &ea,
	...
	_center += ((directionVec * forwardSpeed) + (sideVec * sideSpeed) + (up
			* verticalSpeed)) * dt;
